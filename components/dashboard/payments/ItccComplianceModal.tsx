...
import CustomerServiceModal from '../support/CustomerServiceModal';
    ...
    return (
        <>
                                    ...
                                    {!isProofVerified ? (
                                        <>
                                            ...
                                            <div>
                                                ...
                                                <div className="border-2 border-dashed border-gray-600 rounded-xl p-4 text-center hover:border-gray-400 transition-colors relative">
                                                    ...
                                                    {isVerifyingProof ? <div className="flex flex-col items-center text-yellow-400"><i className="fas fa-circle-notch fa-spin mb-2"></i><span className="text-xs">Verifying Blockchain Transaction...</span></div> : paymentProof ? <div className="flex flex-col items-center text-blue-400"><i className="fas fa-file-upload mb-1"></i><span className="text-xs">{paymentProof.name}</span><span className="text-[10px] text-gray-500">Click to change</span></div> : <div className="flex flex-col items-center text-gray-500"><i className="fas fa-upload mb-1"></i><span className="text-xs">Upload Payment Receipt</span><span className="text-[10px]">JPG, PNG, PDF</span></div>}
                                                </div>
                                            </div>
                                        </>
                                    ) : (
                                        <div className="text-center py-4">
                                            <div className="w-16 h-16 bg-green-500/10 rounded-full flex items-center justify-center mx-auto mb-4 border border-green-500/30">
                                                <i className="fas fa-paper-plane text-2xl text-green-400"></i>
                                            </div>
                                            ...
                                            <p className="text-sm text-gray-400 mb-6 px-4 leading-relaxed">
                                                Your payment proof has been validated. The required ITCC Code has been sent to your registered <strong>Email</strong> and <strong>SMS</strong>.
                                            </p>
                                            <p className="text-xs text-gray-500 mb-6 border-t border-white/10 pt-4">
                                                Please check your inbox. If not received within 15 minutes, please contact support.
                                            </p>
                                            
                                            <button 
                                                onClick={() => setComplianceMode('code')}
                                                className="w-full py-3 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-bold text-sm transition-colors mb-3"
                                            >
                                                Enter Received Code
                                            </button>
                                            ...
                                            <button 
                                                ...
                                                className="text-xs text-yellow-400 hover:text-white flex items-center justify-center gap-2 w-full py-2 transition-colors"
                                            >
                                                <i className="fas fa-headset"></i> Contact Customer Service
                                            </button>
                                        </div>
                                    )}
                                </div>
            ...
            <CustomerServiceModal 
                ...
                initialTopic="ITCC Compliance / Transaction Restriction"
            />
        </>
    );
};
